...
export default function Transaction(props: any): JSX.Element {
    ...
    const getAccList = async (): Promise<any> => {
        try {
            const response = await fetch("http://localhost:8000/account?u_id=" + curUser?.userId, {
                method: "get",
                headers: {"Content-Type": "application/json"},
            });
            // response.json().then((res) => console.log("res", res));
    ...
    const getLedgerList = async (): Promise<any> => {
        try {
            const response = await fetch("http://localhost:8000/accountbook?u_id=" + curUser?.userId, {
                method: "get",
                headers: {"Content-Type": "application/json"},
            });
            // response.json().then((res) => console.log("res", res));
